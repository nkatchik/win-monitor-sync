...
- Windows and the selected monitor use the **same volume percentage**. Windows handles volume keys and its normal volume UI; the app mirrors changes to the monitor. The app shows no volume overlay; the monitor may show its own OSD.
- Left-click or right-click the tray icon to open a compact **Fluent context menu**, with **Brightness** first and **Volume** second. The checkbox beside each name enables that control independently, and stays usable even when its slider is unavailable. **Start with Windows** and **Exit** follow below. The menu follows Windows' app light/dark theme and accent colour. Unavailable sliders stay visible but disabled, without a separate status row or stale percentage. Volume targets the selected monitor speakers; brightness targets the screen under the cursor. Opening the menu reads live levels for enabled controls without changing them. Dragging or using arrow keys adjusts the level without closing it; Escape or clicking elsewhere dismisses it.
- Monitor-button changes are checked every five seconds and update Windows and the tray's displayed level.
- While active, control starts automatically from live readings and aligns both volumes to the lower current percentage. New Windows input during discovery takes precedence. Saved volume levels are never restored. Both Windows and the monitor attenuate audio, so a percentage can sound quieter than with Windows held at maximum, including after switching from another computer.
...
- Quick Settings, volume keys, and the tray slider update Windows immediately. The monitor follows through DDC; Windows is never forced back to 100%. The tray keeps your requested position while the hardware write and readback are pending. If the monitor has coarser steps, matching readback aligns Windows to the actual hardware percentage without overriding newer input.
...
3. Sync starts in the tray without opening a window. Left-click or right-click its icon to use the sliders, enable/disable **Brightness** or **Volume**, or change **Start with Windows**.
4. Try the **Volume** slider or volume keys, then the Dell's volume buttons. Windows and the tray show matching monitor volume after confirmation; allow up to five seconds for monitor-button changes to reach Windows and the menu.
...
Uncheck **Brightness** and **Volume** to stop all monitoring while keeping the tray icon available, or use **Exit** to close the app. The next launch respects each saved checkbox. Turning off **Start with Windows** does not stop the current session. Old saved pairing and pause settings are no longer used.

If DDC fails, the affected slider is disabled and its percentage is cleared while automatic retries continue for that enabled control. Its checkbox remains usable, so you can opt out even when DDC is unavailable. Ordinary Windows volume remains usable. Enable DDC/CI in the monitor's menu and select its speakers in Windows; no pairing configuration is required.
