...
- Windows handles volume keys and displays its normal volume UI; changes are mirrored to monitor volume with no app volume overlay. Brightness media keys control the screen under the cursor and show a Windows-style indicator; fixed shortcuts remain a fallback.
